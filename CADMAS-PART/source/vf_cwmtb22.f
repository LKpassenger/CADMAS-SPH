      SUBROUTINE VF_CWMTB22(KM,WT1,WT2,ZC,UN,UT,DMTBZZ,DMTBUN,DMTBUT)

CD=== 概要 ===========================================================

CDT   VF_CWMTB22:マトリクスデータ-2の流速の計算
...
CD    DMTBUT(MTBZZ,MTBTT) : IN : R*8 : マトリクスデータの鉛直方向流速
      DIMENSION DMTBZZ(MTBZZ2),DMTBUN(MTBZZ2,MTBTT2)
      DIMENSION DMTBUT(MTBZZ2,MTBTT2)
...
CD    -- 位置の検索(部分サーチ) --
      IF     (ZC.LE.DMTBZZ(1)    ) THEN
        ...
        W1=1.0D0
        W2=0.0D0
      ELSEIF (ZC.GE.DMTBZZ(MTBZZ2)) THEN
        KM=MTBZZ2-1
        W1=0.0D0
        W2=1.0D0
      ELSE
        KP=0
        DO 100 K=KM,MTBZZ2-1
 ...
 100    CONTINUE
 110    CONTINUE
        IF (KP.EQ.0) CALL VF_A2ERR('VF_CWMTB22','P.G ERROR.')
        KM=KP
      ENDIF
