      SUBROUTINE VF_OGBCVL(BCVL,WKBC,INDB,NBX,NBY,NBZ)

CD=== 概要 ===========================================================

CDT   VF_OGBCVL: 図化用の境界の値を配列に格納する  Store boundary values for graphic representation in array

C==== 宣言 ===========================================================

C     -- 大域型 --
      IMPLICIT INTEGER(I-N),DOUBLE PRECISION(A-H,O-Z)

C     -- 大域変数 --
      INCLUDE 'VF_A0PRM.h'
      INCLUDE 'VF_AFILEI.h'
      INCLUDE 'VF_ANUMBI.h'
      INCLUDE 'VF_APARAI.h'

CD    -- 引数 --
CD    BCVL(NUMB)        : IN  : I*4 : 物理量の境界値
CD    WKBC(NUMB)        : OUT : I*4 : ワーク配列
CD    INDB (MAXB1,NUMB) : IN  : I*4 : 境界面のインデックス
CD    NBX               : IN  : I*4 : x方向境界面の数
CD    NBY               : IN  : I*4 : y方向境界面の数
CD    NBZ               : IN  : I*4 : z方向境界面の数
      DIMENSION BCVL(NUMB),WKBC(NUMB)
      DIMENSION INDB(MAXB1,NUMB)

C==== 実行 ===========================================================

CD    -- 初期設定 --
      I1=MAX(IGRARA(1)-(MYGIS-1),MYIS)
      J1=MAX(IGRARA(2)-(MYGJS-1),MYJS)
      K1=IGRARA(3)
      I2=MIN(IGRARA(4)-(MYGIS-1),MYIE)
      J2=MIN(IGRARA(5)-(MYGJS-1),MYJE)
      K2=IGRARA(6)
      IX=I2+1
      JY=J2+1
      KZ=K2+1
      NX=0
      NY=NBX
      NZ=NBX+NBY

CD    -- 数を数える --
      DO 100 L=1,NUMB
        IJK=INDB(1,L)
        NS =INDB(2,L)
        K  =(IJK-1)/(NUMI*NUMJ)+1
        IJK=IJK-NUMI*NUMJ*(K-1)
        J  =(IJK-1)/NUMI+1
        I  =IJK-NUMI*(J-1)
        IF     (NS.EQ.1 .OR. NS.EQ.2) THEN
          IF (I1.NE.I2) THEN
            IF (I1.LE.I .AND. I.LE.IX) THEN
              IF (J1.LE.J .AND. J.LE.J2) THEN
                IF (K1.LE.K .AND. K.LE.K2) THEN
                  NX=NX+1
                  WKBC(NX)=BCVL(L)
                ENDIF
              ENDIF
            ENDIF
          ENDIF
        ELSEIF (NS.EQ.3 .OR. NS.EQ.4) THEN
          IF (J1.NE.J2) THEN
            IF (I1.LE.I .AND. I.LE.I2) THEN
              IF (J1.LE.J .AND. J.LE.JY) THEN
                IF (K1.LE.K .AND. K.LE.K2) THEN
                  NY=NY+1
                  WKBC(NY)=BCVL(L)
                ENDIF
              ENDIF
            ENDIF
          ENDIF
        ELSE
          IF (K1.NE.K2) THEN
            IF (I1.LE.I .AND. I.LE.I2) THEN
              IF (J1.LE.J .AND. J.LE.J2) THEN
                IF (K1.LE.K .AND. K.LE.KZ) THEN
                  NZ=NZ+1
                  WKBC(NZ)=BCVL(L)
                ENDIF
              ENDIF
            ENDIF
          ENDIF
        ENDIF
 100  CONTINUE

C     -- 実行文の終了 --
      GOTO 9999

C==== 終了 ===========================================================

 9999 CONTINUE
      RETURN
      END
